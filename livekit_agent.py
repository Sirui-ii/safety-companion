...
async def entrypoint(ctx: JobContext) -> None:
    ...
    sip_caller = await ctx.wait_for_participant(
        kind=rtc.ParticipantKind.PARTICIPANT_KIND_SIP,
    )
    logger.info("SIP caller ready for greeting: %s", sip_caller.identity)
    await session.say(
        (
            "Hey. I'm here with you. "
            "If you're in immediate danger, call emergency services or 988 now. "
            "If you're safe enough to talk, just start with this: what happened?"
        ),
